...
> **חשוב:** זה אינו בנק ואינו שירות פיננסי. אין כסף אמיתי ואין ייעוץ פיננסי.  
...
- התחברות — שם משתמש + סיסמה (בלי אימות מייל או SMS)
- פתיחת חשבון — הזנת פרטים ראשוניים; יתרה/סניף/מספרי כרטיס נוצרים אוטומטית
- עו״ש — יתרה + תנועות (כולל קניות אוטומטיות מכרטיס חיוב)
- ריכוז יתרות — כמה נשאר באמת (עו״ש − חוב אשראי + חסכונות + השקעות)
- כרטיסים — חיוב/אשראי; באשראי מסגרת+חוב+תנועות כרטיס; סימולציית קניות אוטומטית
...
- נתונים מ־mock בקוד (`mockUsers` כזרע); עו״ש וכרטיסים ב־`localStorage` לפי משתמש
- סימולטור קניות אוטומטי (`simulateCardPurchase`) — אותה פונקציה תחליף ל־API/DB בהמשך
- משתמש מחובר ב־`sessionStorage`; העדפות UI ב־`sessionStorage`
